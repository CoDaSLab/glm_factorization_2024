...
for i = 1:length(levels{1})
    for j = 1:length(levels{2})
        X(find(F(:,1) == levels{1}(i) & F(:,2) == levels{2}(j)),:) = simuleMV(reps,vars,8) + repmat(randn(1,vars),reps,1);%#ok
    end
end
...
X_size = size(X);
f_level1 = randi([1,4]); %note
f_level2 = randi([1,3]);
row_indices = find(all(F == [f_level1,f_level2],2)); %only for the selection in f_2
all_columns = 1:X_size(2);
[rowsl,colsl] = ndgrid(row_indices,all_columns);
idxl = sub2ind(X_size,rowsl,colsl);

lvls_actual = zeros(R,length(lvls));
