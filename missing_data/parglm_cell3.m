function [T, parglmo] = parglm(X, F, exchange, interactions, prep, n_perm, ts, ordinal, fmtc, coding)
...
% Conditional Mean Replacement, permutations of individuals.
%
...
% levels.
%
% exchange: [Nx1] index to identify exchangeable units, each unit marked
...
N = size(X, 1);
M = size(X, 2);
if nargin < 3 || isempty(exchange), exchange = []; end;
if nargin < 4 || isempty(interactions), interactions = []; end;
if nargin < 5 || isempty(prep), prep = 2; end;
if nargin < 6 || isempty(n_perm), n_perm = 1000; end;
if nargin < 7 || isempty(ts), ts = 1; end;
if nargin < 8 || isempty(ordinal), ordinal = zeros(1,size(F,2)); end;
...
for i=1:length(ru)
    ...
    for j=1:length(c(ind))
        ...
        if length(ind2)>length(ind3)
            X(r(ind(j)),c(ind(j))) = nanmean(X(ind2,c(ind(j)))); % use the mean of the cell, suggested if possible
        else
            X(r(ind(j)),c(ind(j))) = nanmean(X(:,c(ind(j)))); % use the global mean
        end
    end
end
parglmo.data = X;
...
for j = 1 : n_perm*mtcc
    ...
    perms = zeros(size(X,1),1);
    upp = randperm(length(up));
    for p = 1:length(up)
        ind = find(exchange == up(p));
        indp = find(exchange == upp(p));
        perms(indp) = ind;
    end
    ...
    for i=1:length(ru)
        ...
        for f=1:length(c(ind))
            ...
            if length(ind2)>length(ind3)
                X(r(ind(f)),c(ind(f))) = nanmean(X(ind2,c(ind(f)))); % use the mean of the cell, suggested if possible
            else
                X(r(ind(f)),c(ind(f))) = nanmean(X(:,c(ind(f)))); % use the global mean
            end
        end
    end
